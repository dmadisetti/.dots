...
pub fn preinstall(
    ...
    let disks = utils::lsblk()?;

    defaults["installation"] = json!({
        "installation_zfs": {
            "installation_zfs_available": disks.clone()
        },
        "installation_disko": {
            "installation_disko_available": disks.clone()
        },
    });
    ...
    // If we are not doing an installation, we can skip the rest
    if data["installation"].as_bool().unwrap_or(false) {
        // Set up general config with new host data
