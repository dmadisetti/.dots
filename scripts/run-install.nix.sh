...
if [ "$dont_refresh" != "true" ] && [ "$install" != "true" ]; then
  mkdir -p $DOTFILES/nix/sensitive
  cp $tmp_install_dir/sensitive.nix \
    $DOTFILES/nix/sensitive/flake.nix
  echo "created sensitive flake in $DOTFILES/nix/sensitive." 1>&2
fi

if [ "$install" != "true" ]; then
  echo "installation not specified. exiting." 1>&2
  exit 1
fi

if [ $USER != root ]; then
  echo 'you must be root to proceed with installation' 1>&2
  echo 'however, you may restart installation by running' 1>&2
  echo "sudo $tmp_install_dir/provision.sh. You can also" 1>&2
  echo "use this time to go over the provisioning configs" 1>&2
  echo "in this directory." 1>&2
  exit 1
fi
...
if [ "$has_disko" == "true" ]; then
  nix run github:dmadisetti/disko/install#disko -- --mode disko --root-mountpoint /mnt $tmp_install_dir/install.nix || exit 1
else
  legacy_zfs
fi
